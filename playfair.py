...
class StringManip:
    ...
    def cleanString(self,s,options = {'up':1,'reNonAlphaNum':1,'reSpaces':'','spLetters':'X'}):
        ...
        @returns string -- cleaned message1
...
class PlayFair:
    ...
    def getDcryptionDigraph(self,digraph):
        """
        Turns a given digraph into its encoded digraph whether its on
        the same row, col, or a square
        @param   list -- digraph
...
while(flag == 1):
        if (task== '1'):
            ...
            add = "";
            print ("Playfair Encryption Tool (P.E.T)\n Written By:Vamshider Reddy,Dasari\n****************************")
            print("The Encrypted messege is:")
         
            while(a<len(newMessage)-1):
                m=newMessage[a]
                n=newMessage[a+1]
                encodedCode =''.join( myCipher.getCodedDigraph([m,n]))
                add = add +encodedCode
                a=a+2
            print(add)
            print ('**************************\n')
            ...
            while(a<len(newMessage)-1):
                m=newMessage[a]
                n=newMessage[a+1]
                encodedCode =''.join(myCipher.getDcryptionDigraph([m,n]))
                add=add+encodedCode
                add=add.replace("X","")
                a=a+2
            print(add)
